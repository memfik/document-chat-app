...
export default function OSKContractsForm({ title, description }: { title: string; description: string }) {
    ...
    <Paper elevation={3} sx={{ p: 2.5, maxWidth: 512 }}>
      ...
      <Box component="form" onSubmit={handleSubmit} display="flex" flexDirection="column" gap={2}>
        <Box display="grid" gridTemplateColumns="1fr 1fr" gap={1.5}>
          <TextField select size="small" fullWidth label="Инициатор" name="initiator" defaultValue="">
            <MenuItem value=""><em>— Выберите —</em></MenuItem>
          </TextField>
          <TextField select size="small" fullWidth label="Центр / Децентр" name="centerDecentr" defaultValue="">
            <MenuItem value=""><em>— Выберите —</em></MenuItem>
          </TextField>
        </Box>
