...
export default function BudgetNonExecutionForm({ title, description }: { title: string; description: string }) {
    ...
    <Paper elevation={2} sx={{ p: 2.5, maxWidth: 640 }}>
      ...
      <Box component="form" onSubmit={handleSubmit} display="flex" flexDirection="column" gap={2}>
        <Box display="grid" gridTemplateColumns="repeat(3, 1fr)" gap={1.5}>
          ...
          <TextField select size="small" fullWidth label="Статус позиций бюджета" name="budgetPositionStatus" defaultValue="">
            <MenuItem value=""><em>— Выберите —</em></MenuItem>
          </TextField>
        </Box>
